...
if (cooldown == 0) {
    ...
    if(currentRedStreak.length > 7){
        ...
        cooldown = 7;
        Bettable1 = false;
    }else if(currentRedStreak < 6 && currentRedStreak>2){
...
engine.on('GAME_ENDED', function () {
    ...
    if (isBettingNow) {
        if (!gameInfos.cashedAt) {
            ...
            log(allgames[-1].bust);//See the array and check last 2-3 games and up the bet if those are win
